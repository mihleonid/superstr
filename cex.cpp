...
thread_local ll comp_size[maxstrn];
...
string solve_greedy(const vector<string> &strs, vector<ov_edge> &edges) {
    ll n = strs.size();
    vector<ll> next(n, -1);
    vector<bool> free_out(n, true);
    vector<bool> free_in(n, true);
    iota(parent, parent + n, 0);
    fill(comp_size, comp_size + n, 1);

    stable_sort(edges.begin(), edges.end(), greater<>());
...
void check_counterexample(vector<string> &strs, mt19937_64 &mt) {
    ...
    for (ll itt = 0; itt < greedy_shuffles; itt++) {
        ...
        auto greedy = solve_greedy(strs, edges);
        for (ll c = '0'; c <= '1'; c++) {
            ll greedy_c = count(greedy.begin(), greedy.end(), c);
            if (opt_cs[c-'0'] * 2 < greedy_c) {
                lock_guard<mutex> guard(cout_mutex);
                printf("\33[2K\r");
                cout << "COUNTEREXAMPLE FOUND: " << fl(greedy_c) / opt_cs[c-'0'] << endl << endl;
                ff << endl << endl;
...
void gen_random(vector<string>& strs, mt19937_64& mt) {
    ...
    strs.erase(unique(strs.begin(), strs.end()));
}
