...
void check_counterexample(vector<string> &strs, mt19937_64 &mt) {
    ...
    for (ll itt = 0; itt < greedy_shuffles; itt++) {
        ...
        fl rate = fl(greedy.length()) / opt.length();
        if (rate > best.load(memory_order_relaxed)) {
            lock_guard<mutex> guard(cout_mutex);
            printf("\33[2K\r");
            cout << fixed;
            cout << "COUNTEREXAMPLE FOUND: " << rate << endl << endl;
            ff << endl << endl;
            ff << rate << endl << endl;
            ...
            best.store(rate, memory_order::relaxed);
        }
    }
}
...
void gen_random_prime(vector<string> &strs, mt19937_64 &mt) {
    strs.clear();

    ll n = uniform_int_distribution<ll>(3, 7)(mt);
    for (ll i = 0; i < n; i++) {
        string str = "abcdef";
        shuffle(str.begin(), str.end(), mt);
        strs.push_back(str.substr(0, uniform_int_distribution<ll>(2, 5)(mt)));
    }
